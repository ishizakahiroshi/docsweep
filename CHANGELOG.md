...
- **様子見の卒業期限と、期限到来分だけの明示昇格。** `plan_*.md` / `bugfix_*.md` を
  `[様子見]` へ移すとき、その日から既定 3 日後を卒業期限として frontmatter の `due` に入れる
  （`due.default_offset_days.plan_watching` / `bugfix_watching` で変更可）。
  既に `[様子見]` の文書を再指定した場合は既存の `due` を保護する。
  **`due` の到来だけでは archive しない。** 昇格は `promote --due-expired` を明示したときに限り、
  due 到来（当日を含む）かつ `[様子見]` の文書だけを `[完了]` へ進めて archive へ移す。
  `due` 無し・未来 due・不正 due・`never_archive` は対象外。`sweep` は従来どおり
  `[様子見]` を対象にしない（寝かせを守る不変条件は変更なし）。
- **`apply --watching-days N` / MCP `watching_days=N`。** `--to watching` の遷移にだけ
  卒業期限を今日 + N 日で上書きする。設定ファイルは変更しないので、1 本だけ長く寝かせたい
  ときに `.docsweep.yaml` を触らずに済む。
- **卒業判定の条件を CLI / MCP / 看板で揃えた。** 3 経路とも「当日を含む due 到来」で
  同じ対象を示す。看板の「▼卒業判定」セクションも同じ条件になる。
...
- **`closeout-check --to done` の `受入条件` 欠落を blocker から manual check へ降格。**
  `## 受入条件` が無いときの扱いを `missing_section` の blocker ではなく
  `missing_acceptance_section` の manual check にした。blocker のままだと、
  この規約が明文化される前に書かれた plan が機械的に closeout できず、
  「節を足すこと自体が gate 通過のための作業」になっていた（作者環境で 7 本が同じ理由で止まった）。
  `## 完了条件` / `## 検証` の欠落は従来どおり blocker のまま。`--to watching` は変更なし。
- **`plan_*.md` の必須セクションを配布規約へ明記。** `templates/CLAUDE.md` と
  `docs/conventions.md` は `## context配分` しか必須と書いておらず、
  `## 完了条件` / `## 検証` / `## 受入条件` を要求しているのは実装だけだった。
  さらに「見出しに分類語を入れない」の一覧が `完了条件` / `検証` / `受入` を挙げており、
  必須節そのものと矛盾していた。正規セクションを例外として明示した。
...
- **配布フックが必須セクション自体を警告していたのを修正。**
  `templates/.githooks/docsweep-check.py` の分類語チェックは `全体の検証手順` だけを
  例外にしていたため、規約どおりに `## 完了条件` / `## 検証` を書いた plan が
  毎回「H2 見出しに分類語があります」と警告されていた。正規セクション
  （`完了条件` / `検証` / `受入条件` / `変更ファイル` / `変更予定ファイル`）を例外に加えた。
  `### C1 検証` のような非正規の見出しは従来どおり警告する。
...
- **索引の project 単位をスキャンルートからリポジトリ（project_root）へ揃えた。**
  ...
  なる。副作用として、basename が同じスキャンルート（`D:/dev` と `C:/Users/x/dev`）が <!-- secrets-scan: allow -->
