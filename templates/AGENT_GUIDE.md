...
1. **材料を集める**: `docsweep triage --json`（または MCP の `triage` ツール）を呼ぶ。
...
3. **実行する**: `docsweep apply --path <p> --action <a>`（MCP なら `apply` ツール）。
   `action` は **閉じた集合** から選ぶ:
...
- `docsweep sweep` — done/discarded を archive へ一括移送（様子見は守る）。
- `docsweep promote --state watching --to done` — リリース前に様子見をまとめて昇格。
- `docsweep index` / `docsweep pending` — 横断 INDEX 再生成 / 保留だけ表示。
- `docsweep summary` — 要点だけに絞った JSON。コンテキストに載せやすい。
...
変更が要るときは `docsweep inject` で再同期すること。
