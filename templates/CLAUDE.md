# AI 作業ドキュメント運用ルール（ai-docs-sweep 標準）
...
ai-docs-sweep はこのラベルを読み取って自動アーカイブ・要判断フラグを判定します。

| 種別 | 使えるラベル |
|---|---|
| `plan_*.md` | `[計画]`（新規・未着手） / `[実行中]`（一部着手） / `[完了]` |
| `bugfix_*.md` | `[完了]` / `[対応中]` / `[再発調査中]` |
| `pending_*.md` | `[保留]` |

例: `# [計画] 認証フローのリファクタ`

- `> ステータス:` 行は**書かない**（状態は H1 ラベルに集約）。
- 状態が変わったら H1 ラベルを書き換える。

> フロントマター方式を併用したい場合は、H1 ラベルに加えて
> `status: planning | in-progress | done | pending` を front matter に置いてもよい
> （ai-docs-sweep はどちらの検出方式も設定で切り替え可能）。
...
- H1 直下に `> 最終更新: YYYY-MM-DD(曜) HH:MM:SS` を 1 行。
- その下に `## context配分` 表を**必ず先頭セクション**として置く。
  ...
  - 全 C が `plan` なら H1 は `[計画]`、一部 `fix` なら `[実行中]`、全 `fix` なら `[完了]`。
...
- H1 先頭にラベル（修正済み=`[完了]` / 調査中=`[対応中]` / 再発=`[再発調査中]`）。
- H1 直下に `> 最終更新: YYYY-MM-DD(曜) HH:MM:SS` のみ。
...
- `[完了]` になったファイルは ai-docs-sweep が `archive/` へ自動移送する。

---

## 最終更新日時の取得

`> 最終更新:` 行には、記入直前に取得した**実行環境のローカル日時**を
`YYYY-MM-DD(曜) HH:MM:SS` 形式で入れること。推測で書かない。
