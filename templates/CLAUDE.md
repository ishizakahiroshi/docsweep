...
- ラベル語彙はプロジェクト設定（`.docsweep.yaml` の `states:`）で追加・改名・言語追加できる。
  上表は内蔵デフォルト。`docsweep inject` は `states:` からこのラベル節を生成する（設定と検出が常に同期）。
...
新しい AI セッションを始めたら、コードに触る前に **まず `docsweep triage`**
...
- 横断 INDEX 全体を俯瞰したいときは `docsweep summary`、人間向け週次は `docsweep report`。
