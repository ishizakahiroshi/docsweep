...
- **plan**: 先頭に `## context配分` 表（章番号 `C1/C2/C3`、種別は `planned`/`done` の 2 値・旧表記の `plan`/`fix` は同義）。列順は `C` / `種別` / `内容` / `備考/注意点`（2026-08-27 統一。旧い順 `C` / `内容` / `種別` の既存 md も読める＝列は名前と中身で解決している）。リリース plan は `リリース引数` / `実行計画` / `申し送り` の専用構成を使う。
