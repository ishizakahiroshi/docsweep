...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_index_rebuild = sub.add_parser(
        "index-rebuild",
        help="SQLite 索引を全件再構築（files テーブルをクリアしてから全走査）"
    )
...
def cmd_index_sync(args: argparse.Namespace) -> int:
    ...
    cfg = _build_config(args)
    stats = sync_index(cfg, full=False)
    payload = {
...
def cmd_index_rebuild(args: argparse.Namespace) -> int:
    """SQLite 索引を全件再構築。``files`` テーブルをクリアしてから全走査する。"""
    from .scan import sync_index

    cfg = _build_config(args)
    stats = sync_index(cfg, full=True)
    payload = {
        "projects": stats.projects,
        "files_total": stats.files_total,
        "files_added": stats.files_added,
...
_SUBCOMMANDS = {
    ...
    "index-sync", "index-rebuild", "index-watch", "brief", "cross", "capture",
...
_DISPATCH = {
    ...
    "index-watch": cmd_index_watch, "brief": cmd_brief, "cross": cmd_cross,
