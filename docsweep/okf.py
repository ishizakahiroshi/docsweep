...
def _profile_path(version: str) -> Path:
    ...
    if not _VERSION_RE.fullmatch(version):
        raise OkfProfileError(t("okf.invalid_version", version=version))
...
def _decode_profile(raw_bytes: bytes, *, source: str, expected_sha256: str | None) -> OkfProfile:
    ...
    if expected_sha256 and digest.lower() != expected_sha256.strip().lower():
        raise OkfProfileError(
            t("okf.sha256_mismatch", expected=expected_sha256, actual=digest)
        )
    try:
        payload = json.loads(raw_bytes.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise OkfProfileError(t("okf.json_load_failed", source=source)) from exc
    if not isinstance(payload, dict):
        raise OkfProfileError(t("okf.root_not_object"))
    ...
    if not lifecycle_values or len(set(lifecycle_values)) != len(lifecycle_values):
        raise OkfProfileError(t("okf.lifecycle_values_not_unique"))
    default = str(payload.get("lifecycle_default") or "stable").strip().lower()
    if default not in lifecycle_values:
        raise OkfProfileError(t("okf.lifecycle_default_unknown"))
    ...
    if any(value not in lifecycle_values for value in normalized_status_map.values()):
        raise OkfProfileError(t("okf.status_map_outside_lifecycle"))
...
def _read_local_profile(path: Path) -> bytes:
    try:
        ...
        raise OkfProfileError(t("okf.profile_unreadable", path=path)) from exc
    if len(raw) > _MAX_PROFILE_BYTES:
        raise OkfProfileError(t("okf.profile_too_large"))
    return raw
...
def _read_remote_profile(url: str) -> bytes:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise OkfProfileError(t("okf.invalid_url", url=url))
    # 取得内容を固定しない使い方は「今日たまたま取れたルール」で判定することになる。
    # 拒否はしない（明示的な opt-in なので利用者の判断）が、黙って受け入れもしない。
    import sys

    if parsed.scheme == "http":
        print(f"warning: {t('okf.plain_http', url=url)}", file=sys.stderr)
    ...
    try:
        ...
        raise OkfProfileError(t("okf.fetch_failed", url=url)) from exc
    if len(raw) > _MAX_PROFILE_BYTES:
        raise OkfProfileError(t("okf.profile_too_large"))
    return raw
...
def load_okf_profile(
    ...
    *,
    ...
    if sha256 is not None and not _SHA256_RE.fullmatch(sha256.strip()):
        raise OkfProfileError(t("okf.invalid_sha256"))
    ...
    if parsed.scheme in {"http", "https"}:
        if not sha256:
            ...
            print(f"warning: {t('okf.no_sha256', source=source_text)}", file=sys.stderr)
